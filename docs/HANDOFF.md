...
- [x] **`browser-test-execution` 스킬에 "요소 명확화(disambiguation)" 규칙 추가** — 6번 참조
- [x] **명확화 규칙 회귀 테스트 추가** (`tests/disambiguation/`, `npm test`) — 실패 시나리오(`삭제` vs `협력사 삭제`) 재현, 8개 단언 전부 통과로 규칙 검증 완료
- [ ] **(다음 작업 후보)** 실제 프로젝트의 다른 실패 패턴을 회귀 테스트로 추가 / 하이브리드(검증된 셀렉터 캐싱) 구현 / 자율 실행 에이전트 확장(7번)
...
tests/
└── disambiguation/  # 명확화 규칙 회귀 테스트 (fixture.html + *.test.mjs)
```
...
## 6. 요소 명확화 (구체안) ★ — ✅ 구현·검증 완료

> 상태: `browser-test-execution` 스킬에 섹션 추가됨 + 회귀 테스트(`tests/disambiguation/`, `npm test`)로 검증됨. 실패 시나리오(`삭제` vs `협력사 삭제`) 재현 시 순진한 셀렉터는 오클릭/모호, 아래 규칙은 의도대로 정확 선택 확인(8개 단언 통과).

`browser-test-execution` 스킬에 추가된 **"요소 명확화(disambiguation)" 섹션**의 규칙:
